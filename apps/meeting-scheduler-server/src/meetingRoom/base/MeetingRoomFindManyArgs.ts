/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { MeetingRoomWhereInput } from "./MeetingRoomWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { MeetingRoomOrderByInput } from "./MeetingRoomOrderByInput";

@ArgsType()
class MeetingRoomFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => MeetingRoomWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => MeetingRoomWhereInput, { nullable: true })
  @Type(() => MeetingRoomWhereInput)
  where?: MeetingRoomWhereInput;

  @ApiProperty({
    required: false,
    type: [MeetingRoomOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [MeetingRoomOrderByInput], { nullable: true })
  @Type(() => MeetingRoomOrderByInput)
  orderBy?: Array<MeetingRoomOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { MeetingRoomFindManyArgs as MeetingRoomFindManyArgs };
